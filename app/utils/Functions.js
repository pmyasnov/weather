Ext.define('Weather.utils.Functions', {
    singleton: true,
	...
	country information. In that case you wouldnвЂ™t load the positions based on 
	...
	See Weather.utils.Functions.getWeather(city + " " + country);. 
			...
			callback: function(records, operation, success) {
				...
				if (records.length > 0) {
					...
					if (city && country) {
						...
						Weather.utils.Functions.getWeather(city + " " + country);
					} else {
	...
	getCurrentLocation: function() {
		...
		Ext.device.Geolocation.getCurrentPosition({
			...
			success: function(position) {
				...
				//console.log(place);
				Weather.utils.Functions.getWeather(place);
				Ext.Viewport.unmask();
			},
			failure: function() {
				Ext.Viewport.unmask();
	...
	*/
	getWeather: function(place) {
		Ext.data.JsonP.request({
			...
			params: {
				...
				format: 'json',
				num_of_days: '1',
				key: Weather.utils.Functions.API_KEY
			},
			success: function(result, request) {
				...
				try {
					...
					Weather.utils.Functions.createTemplate(weather[0]);
	...
	if the weather code maps to вЂњrainвЂќ (weather code: 265). And there are two custom 
